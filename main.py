...
# ═══════════════════════════════════════════
# CONFIG (env variables থেকে)
# ═══════════════════════════════════════════
TOKEN       = os.environ.get("TOKEN", "8716527870:AAFpgkcAsG5vIhCSlCh_M3v-8BDixyLd1mg")
OWNER_ID    = int(os.environ.get("OWNER_ID", "8917082487"))
CHANNEL_URL = os.environ.get("CHANNEL_URL", "https://t.me/ALAMINOOPOU")

BASE_DIR      = os.environ.get("DATA_DIR", os.path.abspath(os.getcwd()))
UPLOAD_DIR    = os.path.join(BASE_DIR, "downloads/8917082487/uploads")
DOWNLOAD_DIR  = os.path.join(BASE_DIR, "downloads")
DB_FILE       = os.path.join(BASE_DIR, "downloads/8917082487/codes.json")
...
def block_user(uid):
    ...
    # 🔴 Blocked user এর সব রানিং স্ক্রিপ্ট স্টপ করে দাও
    keys_to_stop = [
        k for k, s in running_scripts.items()
        if s["owner"] == uid and s["proc"].poll() is None
    ]
    for k in keys_to_stop:
        s = running_scripts.get(k)
        if not s:
            continue
        try:
            s["proc"].terminate()
        except Exception as e:
            logger.error(f"terminate failed for {k}: {e}")
        try:
            s["log"].close()
        except Exception:
            pass
        running_scripts.pop(k, None)
        logger.info(f"🛑 Stopped script {k} (owner blocked: {uid})")
...
# ✅ নতুন: আনব্লকড ইউজারের জন্য Restart বাটন
def restart_keyboard():
    return ReplyKeyboardMarkup(
        [[KeyboardButton("🔄 Bot Restart")]],
        resize_keyboard=True
    )
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_users":
        ...
        # ✅ আনব্লক করা ইউজারকে মেসেজ + Restart বাটন পাঠাও
        try:
            await context.bot.send_message(
                chat_id=target,
                text=(
                    "✅ <b>আপনাকে আনব্লক করা হয়েছে!</b>\n\n"
                    "🔄 এখন বট রিস্টার্ট করুন — এরপর আবার বট ব্যবহার করতে পারবেন।\n\n"
            ...
            logger.error(f"unblock notify failed for {target}: {e}")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_blocked(user_id):
        ...
        # ✅ Unblocked user যদি "🔄 Bot Restart" চাপে
        if text == "🔄 Bot Restart":
            if not is_blocked(user_id):
                await update.message.reply_text(
                    "✅ আপনি এখন আনব্লকড! বট রিস্টার্ট হয়েছে 🎉\n\n"
                    "নিচের মেনু থেকে কাজ শুরু করুন 👇",
                    reply_markup=show_menu_keyboard(user_id == OWNER_ID)
                )
                await send_welcome(update.message)
            else:
                await update.message.reply_text(
                    "🚫 আপনি এখনো ব্লকড আছেন।\n\n"
    ...
    # ✅ যারা আনব্লক হয়েছেন কিন্তু এখনো approved_users-এ নেই, তাদের জন্যও Bot Restart হ্যান্ডল
    if text == "🔄 Bot Restart":
        if is_blocked(user_id):
            await update.message.reply_text(
                "🚫 আপনি এখনো ব্লকড আছেন।\n\n"
